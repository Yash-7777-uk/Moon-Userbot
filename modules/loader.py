...
async def loadmod(client: Client, message: Message):
    ...
    if len(message.command) > 1:
        ...
        if url.startswith(
            "https://raw.githubusercontent.com/Yash-7777-uk/custom_modules/main/"
        ):
            ...
            try:
                async with (
                    ...
                    session.get(
                        "https://raw.githubusercontent.com/Yash-7777-uk/custom_modules/main/full.txt"
                    ) as resp,
                ):
                    f = await resp.text()
            except Exception:
            ...
            if module_name in modules_dict:
                url = f"https://raw.githubusercontent.com/Yash-7777-uk/custom_modules/main/{modules_dict[module_name]}.py"
            else:
                await message.edit(
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "https://raw.githubusercontent.com/Yash-7777-uk/custom_modules/main/modules_hashes.txt"
                ) as resp:
            ...
            if hashlib.sha256(resp_content).hexdigest() not in modules_hashes:
                return await message.edit(
                    "<b>Only <a href=https://github.com/Yash-7777-uk/custom_modules/tree/main/modules_hashes.txt>"
                    "verified</a> modules or from the official "
                    "<a href=https://github.com/Yash-7777-uk/custom_modules>"
...
async def load_all_mods(client: Client, message: Message):
    ...
    async with aiohttp.ClientSession() as session:
        for module_name in modules_list:
            url = f"https://raw.githubusercontent.com/Yash-7777-uk/custom_modules/main/{module_name}.py"
            async with session.get(url) as resp:
                if resp.status != 200:
                    continue
                content = await resp.read()
...
async def updateallmods(client, message: Message):
    ...
    async with aiohttp.ClientSession() as session:
        for module_file in modules_installed:
            ...
            if module_name in modules_dict:
                async with session.get(
                    f"https://raw.githubusercontent.com/Yash-7777-uk/custom_modules/main/{modules_dict[module_name]}.py"
                ) as resp:
                    if resp.status != 200:
                        continue
                    content = await resp.read()
