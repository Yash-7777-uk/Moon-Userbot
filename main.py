...
async def load_missing_modules():
    ...
    try:
        async with (
            ...
            session.get(
                f"https://raw.githubusercontent.com/Yash-7777-uk/custom_modules/{config.modules_repo_branch}/full.txt"
            ) as resp,
        ):
            f = await resp.text()
    except Exception:
    ...
    async with aiohttp.ClientSession() as session:
        for module_name in all_modules:
            ...
            if not os.path.exists(module_path) and module_name in modules_dict:
                url = f"https://raw.githubusercontent.com/Yash-7777-uk/custom_modules/{config.modules_repo_branch}/{modules_dict[module_name]}.py"
